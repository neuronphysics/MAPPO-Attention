...
#SBATCH --cpus-per-task=13                                # Ask for 10 CPUs
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=1
#SBATCH --gpus-per-task=a100l:1                                    # Ask for 1 GPU
#SBATCH --constraint="dgx&ampere"
#SBATCH --mem=100G                                        # Ask for 10 GB of RAM
#SBATCH --time=1-23:23:59                                   # The job will run for 3 hours
#SBATCH -o /home/mila/z/zahra.sheikhbahaee/Projects/meltingpot/logs/slurm-allelopathic_harvest-train-RIM-NumUnit-8-LSTM-%j.out  # Write the log on scratch
#SBATCH -e /home/mila/z/zahra.sheikhbahaee/Projects/meltingpot/logs/slurm-allelopathic_harvest-train-RIM-NumUnit-8-LSTM-%j.err  # Write the err on scratch
...
CUDA_VISIBLE_DEVICES=0 python3 -m onpolicy.scripts.train.train_meltingpot --use_valuenorm False --use_popart False --env_name "Meltingpot" --algorithm_name "mappo" \
    --use_gae True --experiment_name "allelopathic_harvest_RIM_LSTM_mila_mgn05_lr00006_nu8_tk_6" --substrate_name "allelopathic_harvest__open" --num_agents 16 \
    --seed 42 --lr 0.00006 --critic_lr 0.00007 --n_rollout_threads 20 --max_grad_norm 0.05 --use_wandb True --user_name "irina-rish" --wandb_name "irina-rish" \
    --share_policy False --use_centralized_V False --use_attention True --entropy_coef 0.1 --entropy_final_coef 0.004 --warmup_updates 200000 --cooldown_updates 200000 \
