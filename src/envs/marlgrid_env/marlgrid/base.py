...
class MultiGridEnv(gym.Env):
    ...
    def gen_agent_obs(self, agent):
        ...
        if agent.observation_style=='image':
            ...
            if agent.observe_position:
                ...
                ret['position'] = np.array(agent_pos)/np.array([self.width, self.height], dtype=float)
    ...
    def place_obj(self, obj, top=(0,0), size=None, reject_fn=None, max_tries=1e5):
        ...
        for try_no in range(max_tries):
            pos = self.np_random.integers(low=top, high=bottom)
...
class MultiGridEnv_coodinationNheterogeneity(gym.Env):
    def __init__(
        ...
        '''
        this version allows settings of different levels of coordination among agents when obtaining rewards from the same task 
        '''
...
class MultiGridEnv_PrisonBreakNheterogeneity(gym.Env):
    #this version only allows reward to be collected if there are certain number of agents around the reward(treasure) which is coordination level
    def __init__(
        self,
        agents = [],
        grid_size=None,
    ...
    def reset(self, **kwargs):
        ...
        #self._gen_grid(self.width, self.height)
        self._gen_grid(10, 10)#manually make the size of room smaller to make game easier
    ...
    def step(self, actions):
        ...
        for shuffled_ix in iter_order:
            ...
            if agent.active:
                ...
                # ##count number of agents around the agent
                N_agents_around=0
                Distance_range=2
                x1=agent.pos[0]
                y1=agent.pos[1]
                ...
                for a_idx in range(len(self.agents)):
                    x2=self.agents[a_idx].pos[0]
                    y2=self.agents[a_idx].pos[1]
                    if abs(x2-x1)<=Distance_range and abs(y2-y1)<=Distance_range:
                        N_agents_around+=1

                #only when certain number of agents are around , the agent can pick up a key or open the door
                ...
                # Pick up an object
                    ...
                    if hasattr(cur_cell, 'get_reward') and allow_toggle:
                        rwd = cur_cell.get_reward(agent)
                        if bool(self.reward_decay):
                            rwd *= (1.0-0.9*(self.step_count/self.max_steps))
                        step_rewards[agent_no] += rwd
                    ...
                    if fwd_cell and fwd_cell.can_pickup() and allow_toggle:
                        if agent.carrying is None:
                            agent.carrying = fwd_cell
                            agent.carrying.cur_pos = np.array([-1, -1])
                            self.grid.set(*fwd_pos, None)
...
class MultiGridEnv_keyfortreasuresNheterogeneity(gym.Env):
    #this version only allows reward to be collected if there are certain number of agents around the reward(treasure) which is coordination level
    def __init__(
        self,
        agents = [],
...
class MultiGridEnv_TeamSupportNheterogeneity(gym.Env):
    #this version only allows reward to be collected if there are certain number of agents around the reward(treasure) which is coordination level
    def __init__(
        self,
        agents = [],
            ...
            if agent.active:
                ...
                # Move forward
                    ...
                    if can_move:
                        ...
                        if hasattr(fwd_cell, 'get_reward') and allow_reward:
...
class MultiGridEnv_coodinationNheterogeneityCompoundGoal(gym.Env):
    def __init__(
        self,
        agents = [],
        grid_size=None,
