...
class ACTLayer(nn.Module):
    ...
    def forward(self, x, available_actions=None, deterministic=False):
        ...
        if self.mixed_action :
            ...
            actions = action_logits.mode() if deterministic else action_logits.sample() 
            # Assuming action_logits is a distribution, get log probs for the actions
            action_log_probs = action_logits.log_prob(actions.squeeze(-1))
            #print(f"inside ACTLayer class (act.py) actions (others):{actions} shape {actions.shape}, input {x.shape} action_log_probs: {action_log_probs} shape {action_log_probs.shape}")
