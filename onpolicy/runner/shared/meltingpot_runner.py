...
class MeltingpotRunner(Runner):
    ...
    def run(self):
        ...
        print("Using Shared Policy for Melting Pot!!!")
    ...
    def collect(self, step):
        ...
        value, action, action_log_prob, rnn_states, rnn_cells, rnn_states_critic, rnn_cells_critic \
    ...
    def insert(self, data):
        ...
        rewards = np.array([[player_dict[f'player_{i}'] for i in range(self.num_agents)] for player_dict in rewards], dtype=np.float32)
        if (dones == True).sum() > 0:
            rnn_states[dones == True] = np.zeros(((dones == True).sum(), self.recurrent_N, self.hidden_size), dtype=np.float32) 
            rnn_cells[dones == True] = np.zeros(((dones == True).sum(), self.recurrent_N, self.hidden_size), dtype=np.float32)  
            rnn_states_critic[dones == True] = np.zeros(((dones == True).sum(), *self.buffer.rnn_states_critic.shape[3:]), dtype=np.float32)
            rnn_cells_critic[dones == True] = np.zeros(((dones == True).sum(), *self.buffer.rnn_cells_critic.shape[3:]), dtype=np.float32)
        ...
        self.buffer.insert(new_share_obs, new_obs, rnn_states, rnn_cells, rnn_states_critic, rnn_cells_critic, actions, action_log_probs, values, rewards, masks)
    ...
    def render(self):        
        ...
        for episode in range(self.all_args.render_episodes):
            ...
            rnn_cells = np.zeros((self.n_rollout_threads, self.num_agents, self.recurrent_N, self.hidden_size), dtype=np.float32)
            ...
            for step in range(self.episode_length):
                ...
                self.trainer.prep_rollout()
                action, rnn_states, rnn_cells = self.trainer.policy.act(np.concatenate(obs),
                                                                        np.concatenate(rnn_states),
                                                                        np.concatenate(rnn_cells),
                                                                        np.concatenate(masks),
                                                                        deterministic=True)
                actions = np.array(np.split(_t2n(action), self.n_rollout_threads))
                rnn_states = np.array(np.split(_t2n(rnn_states), self.n_rollout_threads))
                rnn_cells = np.array(np.split(_t2n(rnn_cells), self.n_rollout_threads))
