    ...
    def train(self):
        ...
        for agent_id in torch.randperm(self.num_agents):
            ...
            if self.all_args.algorithm_name == "hatrpo":
                ...
                old_actions_logprob, _, steve_loss = self.trainer[agent_id].policy.actor.evaluate_actions(
                    self.buffer[agent_id].obs[:-1].reshape(-1, *self.buffer[agent_id].obs.shape[2:]),
                    input_rnn_states,
                    self.buffer[agent_id].actions.reshape(-1, *self.buffer[agent_id].actions.shape[2:]),
                    self.buffer[agent_id].masks[:-1].reshape(-1, *self.buffer[agent_id].masks.shape[2:]),
            ...
            if self.all_args.algorithm_name == "hatrpo":
                ...
                new_actions_logprob, _, steve_loss = self.trainer[agent_id].policy.actor.evaluate_actions(
                    self.buffer[agent_id].obs[:-1].reshape(-1, *self.buffer[agent_id].obs.shape[2:]),
                    input_rnn_states,
                    self.buffer[agent_id].actions.reshape(-1, *self.buffer[agent_id].actions.shape[2:]),
                    self.buffer[agent_id].masks[:-1].reshape(-1, *self.buffer[agent_id].masks.shape[2:]),
