...
class RIM(nn.Module):
	def __init__(self, device,
		...
		print("in modularity script, new parameters: ", n_layers, hidden_size, num_units, k, rnn_cell, bidirectional, dropout)
		...
		if self.num_directions == 2:
			self.rimcell = nn.ModuleList([RNNModelRim(rnn_cell, input_size, input_size, [hidden_size], 1,  num_blocks = [num_units], topk = [k], do_gru = rnn_cell == 'GRU', dropout = dropout, version=version, attention_out=attention_out, num_rules = num_rules, rule_time_steps = rule_time_steps, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) if i < 2 else
                                 RNNModelRim(rnn_cell, 2 * hidden_size, 2 * hidden_size, [hidden_size], 1, num_blocks = [num_units], topk = [k], do_gru = rnn_cell == 'GRU', dropout = dropout, version=version, attention_out=attention_out, num_rules = num_rules, rule_time_steps = rule_time_steps, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) for i in range(self.n_layers * self.num_directions)])
		else:
			self.rimcell = nn.ModuleList([RNNModelRim(rnn_cell, input_size, input_size, [hidden_size], 1,  num_blocks = [num_units], topk = [k], do_gru = rnn_cell == 'GRU', dropout = dropout, version=version, attention_out=attention_out, num_rules = num_rules, rule_time_steps = rule_time_steps, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) if i == 0 else
				RNNModelRim(rnn_cell, hidden_size, hidden_size, [hidden_size], 1,  num_blocks = [num_units], topk = [k], do_gru = rnn_cell == 'GRU', dropout = dropout, version=version, attention_out=attention_out, num_rules = num_rules, rule_time_steps = rule_time_steps, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) for i in range(self.n_layers)])
...
class SCOFF(nn.Module):
	def __init__(self,
		...
		self.batch_first = batch_first
		if self.num_directions == 2:
			self.rimcell = nn.ModuleList([RNNModelScoff(rnn_cell, input_size, input_size, hidden_size, 1,  n_templates = num_templates, num_blocks = num_units, update_topk = k, use_gru = rnn_cell == 'GRU', dropout = dropout, version=version, attention_out=attention_out, num_rules = num_rules, rule_time_steps = rule_time_steps, perm_inv = perm_inv, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) if i < 2 else
			...
			self.rimcell = nn.ModuleList([RNNModelScoff(rnn_cell, input_size, input_size, hidden_size, 1, n_templates = num_templates, num_blocks = num_units, update_topk = k, use_gru = rnn_cell == 'GRU', dropout = dropout, version=version, attention_out=attention_out, num_rules = num_rules, rule_time_steps = rule_time_steps, perm_inv = perm_inv, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) if i == 0 else
				RNNModelScoff(rnn_cell, hidden_size, hidden_size, hidden_size, 1, n_templates = num_templates,  num_blocks = num_units, update_topk = k, use_gru = rnn_cell == 'GRU', dropout = dropout, num_rules = num_rules, version=version, attention_out=attention_out,rule_time_steps = rule_time_steps, perm_inv = perm_inv, application_option = application_option, step_att = step_att, rule_selection = rule_selection).to(self.device) for i in range(self.n_layers)])
