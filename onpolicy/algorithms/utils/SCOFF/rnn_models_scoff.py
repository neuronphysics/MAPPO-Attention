    ...
    def init_blocks(self):
        ...
        if True:
            if True:
                ...
                bc_lst.append(
                    BlocksCore(nhid + ninp, nhid, 1, num_blocks, topk, memorytopk, step_att, num_modules_read_input,
                               inp_heads, do_gru=use_gru,
                               do_rel=do_rel, perm_inv=perm_inv, device=device, n_templates=n_templates,
                               share_inp=share_inp, share_comm=share_comm, memory_mlp=memory_mlp,
                               memory_slots=memory_slots, num_memory_heads=num_memory_heads,
                               memory_head_size=memory_head_size, attention_out=attention_out,
    ...
    def forward(self, input, hidden, message_to_rule_network=None):
        ...
        if True:
            ...
            for idx_layer in range(0, self.nlayers):
                ...
                if self.do_rel:
                    self.bc_lst[idx_layer].reset_relational_memory(input.shape[0])
                ...
                for idx_step in range(timesteps):  # input shape: timesteps, batch_size, hidden
                    ...
                    hx, cx, mask, bmask, temp_attn, entropy_ = self.bc_lst[idx_layer](layer_input[idx_step], hx, cx,
                                                                                      idx_step,
                                                                                      do_print=do_print,
