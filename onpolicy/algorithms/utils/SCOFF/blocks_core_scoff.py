...
class BlocksCore(nn.Module):
    ...
    def __init__(self,
        ...
        if do_gru:
            print('USING GRU!')
            if n_templates != 0:
                self.block_lstm = SharedBlockGRU(self.att_out * self.num_blocks_out, self.nhid, k=self.num_blocks_out,
            ...
            print('USING LSTM!')
            if n_templates != 0:
                self.block_lstm = SharedBlockLSTM(self.att_out * self.num_blocks_out, self.nhid, k=self.num_blocks_out,
                                                  n_templates=n_templates)
            else:
                print('Using Normal RIMs')
        ...
        if self.do_rel:
            memory_key_size = 32
            gate_style = 'unit'
            print('gate_style is', gate_style, memory_slots, num_memory_heads, memory_head_size, memory_key_size,
                  memory_mlp)
    ...
    def forward(self, inp, hx, cx, step, do_print=False, message_to_rule_network=None):
        ...
        mask = mask.reshape((inp_use.shape[0], self.num_blocks_out, 1)).repeat((1, 1, self.block_size_out)).reshape(
        ...
        if self.set_transformer:
            hx = hx.reshape((hx.shape[0], self.num_blocks_out, self.block_size_out)).reshape(
                (hx.shape[0] * self.num_blocks_out, self.block_size_out))
            hx = self.set(hx).reshape((batch_size, self.num_blocks_out, self.block_size_out)).reshape(batch_size,
                                                                                                      self.nhid)
