...
def train_qsa(args):
    ...
    for ep in tqdm(range(args.slot_train_ep)):
        for idx, batch_data in enumerate(train_loader):
            ...
            consistency_loss = out['loss']['compositional_consistency_loss'] * ep / len(train_loader.dataset)
            optimizer.zero_grad()
            loss = mse_loss + cross_entropy

            if args.use_orthogonal_loss:
                loss += similarity_loss * args.orthogonal_loss_coef
        ...
        if ep != 0 and ep % 1 == 0:
            with torch.no_grad():
                for idx, batch_data in enumerate(val_loader):
                    ...
                    if ep % args.slot_log_fre == 0 and ('recon' in out) and ('pred_image' in out):
                        masked_image, combined_mask, recon_row = visualize_img(out, batch_data)
