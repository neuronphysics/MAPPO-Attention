...
class RuleNetwork(nn.Module):
    def __init__(self, hidden_dim, num_variables, num_rules=4, rule_dim=64, query_dim=32, value_dim=64, key_dim=32,
                 num_heads=4, dropout=0.1, design_config=None):
        super().__init__()
        ...
        self.variable_rule_select = SelectAttention(rule_dim, hidden_dim, d_k=32, num_read=num_rules,
        ...
        self.variables_select = MultiHeadAttention(n_head=4, d_model_read=hidden_dim, d_model_write=hidden_dim,
                                                   d_model_out=hidden_dim, d_k=32, d_v=32, num_blocks_read=1,
                                                   num_blocks_write=num_variables, topk=3, grad_sparse=False)

        self.variables_select_1 = SelectAttention(hidden_dim, hidden_dim, d_k=16, num_read=1, num_write=num_variables)

        self.phase_1_mha = MultiHeadAttention(n_head=1, d_model_read=2 * hidden_dim * num_variables,
                                              d_model_write=hidden_dim, d_model_out=hidden_dim, d_k=64, d_v=64,
                                              num_blocks_read=1, num_blocks_write=num_rules, topk=num_rules,
                                              grad_sparse=False)
        ...
        self.phase_2_mha = MultiHeadAttention(n_head=1, d_model_read=hidden_dim, d_model_write=hidden_dim,
                                              d_model_out=hidden_dim, d_k=32, d_v=32, num_blocks_read=num_comb,
                                              num_blocks_write=1, topk=1, grad_sparse=False)
        ...
        self.r = r
        self.variable_combinations = torch.combinations(torch.tensor([i for i in range(num_variables)]), r=r,
                                                        with_replacement=True)
        self.variable_combinations_mlp = MLP(r * hidden_dim, hidden_dim)
        self.variable_rule_mlp = MLP(3 * hidden_dim, hidden_dim)
        self.selecter = SelectAttention(hidden_dim, hidden_dim, d_k=16, num_read=num_rules,
        ...
        self.selecter_2 = SelectAttention(hidden_dim, hidden_dim, d_k=16, num_read=1,
                                          num_write=len(self.variable_combinations))
    ...
    def forward(self, hidden, message_to_rule_network=None):
        ...
        if False:
            ...
            if self.design_config['application_option'] == 0 or self.design_config['application_option'] == 1 or \
                    str(self.design_config['application_option']).split('.')[2] == '0':
        ...
        if self.design_config['application_option'] == 0 or self.design_config['application_option'] == 1 or \
            ...
            attn_output, attn_output_weights = self.multihead_attention(transformer_out, transformer_out,
                                                                        transformer_out)
            transformer_out = transformer_out.transpose(0, 1)
            variable_rule = attn_output_weights[:, start_index[-2]:start_index[-2] + num_variables, start_index[-1]:]
            rule_variable = attn_output_weights[:, start_index[-1]:,
                            start_index[-2]: start_index[-2] + num_variables].transpose(1, 2)
        ...
        if self.training:
            ...
            self.rule_probabilities.append(
                torch.softmax(scores.reshape(batch_size, -1), dim=1).reshape(batch_size, num_variables,
                                                                             num_rules).clone().detach())
            entropy = 0
            mask_print = mask
        if self.design_config['application_option'] == 0:
            ...
            relevant_variables_mask = torch.nn.functional.gumbel_softmax(relevant_variables_attn, dim=1, tau=0.5,
                                                                         hard=True)
            ...
            _, phase_1_attn, _ = self.phase_1_mha(read_input.view(read_input.size(0), -1).unsqueeze(1), rule_emb,
                                                  rule_emb)
            ...
            variables_extract = variables_repeat[:, variable_combinations.view(-1),
                                :]  # (B, r * num_combinations, variable_dim)

            variables_extract = list(
                torch.split(variables_extract, self.r, dim=1))  # [(B, r, variable_dim) * num_combinations]

            # Find a way to avoid loop!
            variables_extract = [v.view(variables.size(0), -1) for v in
                                 variables_extract]  # [(B, r * variable_dim ) * num_combinations]
            ...
            combined_variable_representations = self.variable_combinations_mlp(
                variables_extract)  # (B, num_combinations, variable_dim)
            ...
            merged_variable_representation = combined_variable_representations.reshape(
                combined_variable_representations.size(0), -1)  # (B, num_combinations * variable_dim)
            ...
            if self.design_config['selection'] == 'gumble' and self.training:
                mask_variable = torch.nn.functional.gumbel_softmax(scores_2, dim=1, tau=self.gumble_temperature,
                                                                   hard=True)
            else:
            ...
            self.variable_activation.append(
                variable_combinations[torch.argmax(mask_variable, dim=1).detach()].cpu().numpy())
            ...
            temp_index = torch.zeros(combined_variable.size(0), self.r).to(combined_variable.device) + torch.arange(0,
                                                                                                                    combined_variable.size(
                                                                                                                        0)).to(
                combined_variable.device).unsqueeze(-1)
            ...
            penultimate_representation = self.variable_rule_mlp(
            ...
            residual_application_mask[temp_index.long(), combinations.long()] = residual_application_mask[
                                                                                    temp_index.long(), combinations.long()] + variable_rule
            ...
            if self.design_config['selection'] == 'gumble' and self.training:
                mask = torch.nn.functional.gumbel_softmax(scores.view(rules.size(0), -1), dim=1,
                                                          tau=self.gumble_temperature, hard=True).view(-1, num_rules,
                                                                                                       num_combinations)
            else:
            ...
            self.variable_activation.append(
                variable_combinations[torch.argmax(mask_variable, dim=1).detach()].cpu().numpy())
            self.rule_probabilities.append(
                torch.softmax(scores.clone().detach().view(rules.size(0), -1), dim=1).cpu().numpy())
            ...
            combined_variable = combined_variable.repeat(1, self.r, 1)
            rule = rule.repeat(1, self.r, 1)
            variable_rule_representations = torch.cat((selected_variables, combined_variable, rule),
                                                      dim=2)  # (B, 2, 3 * variable_dim)
            variable_rule_representations = variable_rule_representations.view(
                variable_rule_representations.size(0) * self.r, -1).unsqueeze(1)
    ...
    def reset_bias(self):
        self.biases = np.zeros((self.num_rules, self.num_variables))
...
#     def forward(self, hidden, message_to_rule_network=None):
#         # if not self.design_config['grad']:
#         application_option_split = str(self.design_config['application_option']).split('.')
#         if len(application_option_split) >= 2 and application_option_split[1] == '0':
#             hidden = hidden.detach()
#
#         if False and message_to_rule_network.ndim != hidden.ndim:
#             print('dimension of hidden state and message to rule network dont match. Expected both to be 3')
...
#                 and application_option_split[2] == '-1':
#             message_to_rule_network = message_to_rule_network.unsqueeze(1)
#             scores = self.dummy_rule_selector(message_to_rule_network, rule_emb)
#             scores = scores.squeeze(1)
#             if self.training:
...
#         else:
#             if self.design_config['application_option'] == 0 or self.design_config['application_option'] == 1 or \
#                     (len(application_option_split) >= 3 and application_option_split[2] == '0'):
#                 start_index = [0, num_variables]
#                 transformer_input = torch.cat((hidden, rule_emb), dim=1)
#
#         if self.design_config['application_option'] == 0 or self.design_config['application_option'] == 1 or \
