...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_head, d_model_read, d_model_write, d_model_out, d_k, d_v, num_blocks_read, num_blocks_write,
                 topk, grad_sparse, residual=True, dropout=0.1, skip_write=False, flag=False):
        super().__init__()
    ...
    def forward(self, q, k, v, mask=None):
        ...
        sz_b_q, len_q, _ = q.size()
        sz_b_k, len_k, _ = k.size()
        sz_b_v, len_v, _ = v.size()

        residual = q

        q = self.GLN_qs(q).view(sz_b_q, len_q, n_head, d_k)
        # q = self.w_qs(q).view(sz_b, len_q, n_head, d_k)
        k = self.GLN_ks(k).view(sz_b_k, len_k, n_head, d_k)
        v = self.GLN_vs(v).reshape(sz_b_v, len_v, n_head, d_v)
        ...
        output = output.view(n_head, sz_b_q, len_q, d_v)
        output = output.permute(1, 2, 0, 3).contiguous().view(sz_b_q, len_q, -1)  # b x lq x (n*dv)
