...
class BlocksCore(nn.Module):
    ...
    def __init__(self, ninp, nhid, num_blocks_in, num_blocks_out, topkval, step_att, do_gru, num_modules_read_input=2,
                 device=torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu'),
                 version=0, attention_out=32, num_rules=0, rule_time_steps=0, application_option=3,
        ...
        self.mha = MultiHeadAttention(n_head=4, d_model_read=self.block_size_out, d_model_write=self.block_size_out,
                                      ...
                                      topk=self.num_blocks_out, grad_sparse=False)
        ...
        if self.version:
            ...
            self.inp_att = MultiHeadAttention(n_head=1, d_model_read=self.block_size_out,
                                              ...
                                              dropout=0.1, topk=self.num_blocks_in + 1, grad_sparse=False,
                                              skip_write=True)
    ...
    def forward(self, inp, hx, cx, step, do_print=False, do_block=True, message_to_rule_network=None):
        ...
        if do_block:
            if self.step_att:
                ...
                hx_new_grad_mask = blocked_grad.apply(hx_new, mask.reshape(
        ...
        hx_old=hx_old.to(self.device)
        cx_old=cx_old.to(self.device)
