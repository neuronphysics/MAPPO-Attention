...
from onpolicy.algorithms.utils.dinosaur.ocl.ocl_typing import *
...
class FeatureExtractor(nn.Module, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def forward(self, inputs: ImageOrVideoFeatures) ->FeatureExtractorOutput:
        pass
...
class ImageFeatureExtractor(FeatureExtractor):
    ...
    This is implemented by reshaping the frame dimension into the batch dimension and
    inverting the process after extraction of the features.
    ...
    def forward_images(
        ...
        Tuple[ImageFeatures, Positions],
        Tuple[ImageFeatures, Positions, Dict],
    ]:
    ...
    def forward(self, video: ImageOrVideoData) -> FeatureExtractorOutput:
        ...
        if isinstance(result, (Tuple, List)):
            ...
            return FeatureExtractorOutput(result, None, None)
        ...
        return FeatureExtractorOutput(features, positions, aux_features)


def cnn_compute_positions_and_flatten(
    features: CNNImageFeatures,
) -> Tuple[ImageFeatures, Positions]:
...
def transformer_compute_positions(
    features: TransformerImageFeatures,
) -> Positions:
