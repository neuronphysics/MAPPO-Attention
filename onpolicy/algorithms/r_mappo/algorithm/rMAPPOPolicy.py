...
class R_MAPPOPolicy:
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_cells_actor, rnn_states_critic, rnn_cells_critic, masks, available_actions=None,
        ...
        cent_obs = torch.as_tensor(cent_obs).to(self.device)
        obs = torch.as_tensor(obs).to(self.device)
        rnn_states_actor = torch.as_tensor(rnn_states_actor).to(self.device)
        rnn_cells_actor = torch.as_tensor(rnn_cells_actor).to(self.device)
        rnn_states_critic = torch.as_tensor(rnn_states_critic).to(self.device)
        rnn_cells_critic = torch.as_tensor(rnn_cells_critic).to(self.device)
        masks = torch.as_tensor(masks).to(self.device)

        if available_actions is not None:
            available_actions = torch.as_tensor(available_actions).to(self.device)
